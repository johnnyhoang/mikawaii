...
-- Quà của TỪNG giáo viên (mkw_class_rewards) tiếp tục được clone từ danh sách này
...
-- thấy mkw_class_rewards của giáo viên đó.
...
-- 2. Nếu DB cũ còn mkw_tutor_rewards (bản nhân theo từng học sinh) → gộp thành 1 danh sách
--    chung theo tiêu đề (title), lấy bản mới nhất mỗi tiêu đề.
DO $$
BEGIN
    IF to_regclass('public.mkw_tutor_rewards') IS NOT NULL THEN
        INSERT INTO mkw_school_reward_templates (id, title, cost_ruby, quantity, remaining_quantity, created_at)
        SELECT DISTINCT ON (LOWER(title))
            'sch-rew-' || md5(LOWER(title)),
            title, cost_ruby, quantity, remaining_quantity, timestamp
        FROM mkw_tutor_rewards
        ORDER BY LOWER(title), timestamp DESC
        ON CONFLICT (id) DO NOTHING;
    END IF;
END $$;

-- 3. Trỏ lại reward_id của các redemption cũ (đang trỏ vào mkw_tutor_rewards) sang bản ghi
...
BEGIN
    IF to_regclass('public.mkw_tutor_rewards') IS NOT NULL THEN
        UPDATE mkw_reward_redemptions r
        SET reward_id = 'sch-rew-' || md5(LOWER(t.title))
        FROM mkw_tutor_rewards t
        WHERE r.reward_id = t.id
          AND EXISTS (SELECT 1 FROM mkw_school_reward_templates s WHERE s.id = 'sch-rew-' || md5(LOWER(t.title)));
    END IF;
END $$;
...
DO $$
BEGIN
    IF EXISTS (SELECT FROM pg_constraint WHERE conname = 'mkw_reward_redemptions_reward_id_fkey') THEN
    ...
    WHERE reward_id IS NOT NULL
      AND reward_id NOT IN (SELECT id FROM mkw_school_reward_templates);

    ALTER TABLE mkw_reward_redemptions
        ADD CONSTRAINT mkw_reward_redemptions_reward_id_fkey
        FOREIGN KEY (reward_id) REFERENCES mkw_school_reward_templates(id) ON DELETE SET NULL;
END $$;

-- 5. Xoá bảng cũ (đã nhân bản theo từng học sinh — không còn phù hợp kiến trúc "1 danh sách chung").
DROP TABLE IF EXISTS mkw_tutor_rewards;

-- 6. Nhập lần đầu (one-time import) danh mục quà đang hardcode trong code vào DB — CHỈ chạy nếu
--    bảng đang trống (DB hoàn toàn mới, chưa từng có mkw_tutor_rewards/mkw_parent_rewards).
...
WHERE NOT EXISTS (SELECT 1 FROM mkw_school_reward_templates);
