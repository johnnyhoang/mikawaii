...
--   2) Cờ class_rewards_seeded trên mkw_users — nhân bản quà trường -> quà lớp CHỈ 1 LẦN
...
ALTER TABLE mkw_school_reward_templates ADD COLUMN IF NOT EXISTS is_unlimited BOOLEAN NOT NULL DEFAULT FALSE;
ALTER TABLE mkw_class_rewards ADD COLUMN IF NOT EXISTS is_unlimited BOOLEAN NOT NULL DEFAULT FALSE;
ALTER TABLE mkw_users ADD COLUMN IF NOT EXISTS class_rewards_seeded BOOLEAN NOT NULL DEFAULT FALSE;
...
UPDATE mkw_school_reward_templates SET is_unlimited = TRUE WHERE quantity >= 999999;
...
UPDATE mkw_class_rewards SET is_unlimited = TRUE WHERE quantity >= 999999;
...
UPDATE mkw_users u SET class_rewards_seeded = TRUE
WHERE EXISTS (SELECT 1 FROM mkw_class_rewards cr WHERE cr.teacher_id = u.id);
