...
DO $$
BEGIN
  IF to_regclass('public.mkw_gatekeeper_history') IS NOT NULL THEN
    INSERT INTO mkw_riddle_history (profile_id, mode, question_id, used_at)
    SELECT student_id, 'ruby-riddle', question_id, used_at
    FROM mkw_gatekeeper_history legacy
    WHERE NOT EXISTS (
      SELECT 1
      FROM mkw_riddle_history current_history
    ...
    DROP TABLE mkw_gatekeeper_history;
  END IF;
END $$;
