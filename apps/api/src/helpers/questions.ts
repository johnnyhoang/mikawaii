  ...
  if (!lessonId) {
    try {
      const lessonRes = await pool.query(
        'SELECT id FROM mkw_lessons WHERE category = $1 AND grade_tier = $2 AND subject = $3 LIMIT 1',
...
/**
 * Nhân bản Danh Mục Quà Khuyến Học của TRƯỜNG (mkw_school_reward_templates — bảng dùng
 * chung toàn viện, quản lý qua /api/admin/school-rewards) vào danh mục riêng của một
 * giáo viên (mkw_class_rewards). Gọi khi hồ sơ giáo viên được tạo (routes/profiles.ts).
 *
 * Chỉ nhân bản ĐÚNG 1 LẦN trong đời hồ sơ — dùng cờ `mkw_users.class_rewards_seeded`,
...
export const ensureDefaultClassRewards = async (teacherId: string) => {
  const userRes = await pool.query('SELECT class_rewards_seeded FROM mkw_users WHERE id = $1', [teacherId]);
  if (userRes.rows[0]?.class_rewards_seeded) return;

  const templatesRes = await pool.query(
    'SELECT title, cost_ruby, quantity, is_unlimited FROM mkw_school_reward_templates ORDER BY created_at LIMIT 1'
  );
  ...
  await pool.query('UPDATE mkw_users SET class_rewards_seeded = TRUE WHERE id = $1', [teacherId]);
};
