  ...
  try {
    ...
    if (newTeacherId) {
      ...
      const schoolRedemptionsRes = await client.query(
        "SELECT * FROM mkw_reward_redemptions WHERE user_id = $1 AND status = 'pending'",
        [studentId]
      );
      ...
      for (const row of schoolRedemptionsRes.rows) {
        ...
        const classRewardRes = await client.query(
          "SELECT id FROM mkw_class_rewards WHERE teacher_id = $1 AND LOWER(title) = LOWER($2)",
        ...
        if (classRewardRes.rowCount && classRewardRes.rowCount > 0) {
          ...
          await client.query(
            `INSERT INTO mkw_class_rewards (id, teacher_id, title, cost_ruby, quantity, remaining, created_at)
        ...
        await client.query(
          `INSERT INTO mkw_class_reward_redemptions (id, class_reward_id, student_id, reward_title, cost_ruby, status, requested_at)
        ...
        await client.query(
          "DELETE FROM mkw_reward_redemptions WHERE id = $1",
          [row.id]
        );
      }
      ...
      const classRedemptionsRes = await client.query(
        "SELECT * FROM mkw_class_reward_redemptions WHERE student_id = $1 AND status = 'pending'",
        [studentId]
      );
      ...
      for (const row of classRedemptionsRes.rows) {
        ...
        await client.query(
          "UPDATE mkw_class_reward_redemptions SET class_reward_id = $1 WHERE id = $2",
      ...
      // Không còn seed catalog riêng cho học sinh — mkw_school_reward_templates là danh
      ...
      for (const row of classRedemptionsRes.rows) {
        ...
        const schoolRewardRes = await client.query(
          "SELECT id FROM mkw_school_reward_templates WHERE LOWER(title) = LOWER($1)",
          [row.reward_title]
        );
        ...
        await client.query(
          `INSERT INTO mkw_reward_redemptions (id, user_id, reward_id, reward_title, cost_ruby, status, timestamp)
           VALUES ($1, $2, $3, $4, $5, 'pending', $6)
           ON CONFLICT (id) DO NOTHING`,
        ...
        await client.query(
          "DELETE FROM mkw_class_reward_redemptions WHERE id = $1",
          [row.id]
        );
      }
    }
