...
export const checkStudentManagementPermission = async (
  ...
  for (const actorProfile of actorCheck.rows) {
    ...
    if (role === 'secondary_tutor') {
      const linkCheck = await pool.query(
        "SELECT secondary_permissions FROM mkw_class_links WHERE tutor_id = $1 AND student_id = $2 AND link_type = 'secondary' AND status = 'active'",
        [profileId, studentUserId]
      );
...
export const logAuditEvent = async (actorId: string, action: string, targetId: string | null, payload: any = {}) => {
  try {
    const actorRes = await pool.query('SELECT name, role FROM mkw_users WHERE id = $1', [actorId]);
    if (actorRes.rows.length === 0) return;
    const { name, role } = actorRes.rows[0];

    let targetName = null;
    if (targetId) {
      const targetRes = await pool.query('SELECT name FROM mkw_users WHERE id = $1', [targetId]);
    ...
    await pool.query(
      `INSERT INTO mkw_audit_logs (id, actor_id, actor_name, actor_role, action, target_id, target_name, payload)
